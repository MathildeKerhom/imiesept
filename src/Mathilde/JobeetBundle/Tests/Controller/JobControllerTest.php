<?php
namespace Mathilde\JobeetBundle\Tests\Controller;
...
class JobControllerTest extends WebTestCase
{
    ...
    public function setUp()
    {
        ...
        $loader->loadFromDirectory(static::$kernel->locateResource('@MathildeJobeetBundle/DataFixtures/ORM'));
    ...
    public function getMostRecentProgrammingJob()
    {
        ...
        $query = $em->createQuery('SELECT j from MathildeJobeetBundle:Job j LEFT JOIN j.category c WHERE c.slug = :slug AND j.expires_at > :date ORDER BY j.created_at DESC');
    ...
    public function getExpiredJob()
    {
        ...
        $query = $em->createQuery('SELECT j from MathildeJobeetBundle:Job j WHERE j.expires_at < :date');            
        $query->setParameter('date', date('Y-m-d H:i:s', time()));
        $query->setMaxResults(1);
    ...
    public function testIndex()
    {
        ...
        $this->assertEquals('MathildeJobeetBundleControllerJobController::indexAction', $client->getRequest()->attributes->get('_controller'));
        ...
        $this->assertEquals('MathildeJobeetBundleControllerJobController::showAction', $client->getRequest()->attributes->get('_controller'));
    ...
    public function testJobForm()
    {
        ...
        $this->assertEquals('MathildeJobeetBundleControllerJobController::newAction', $client->getRequest()->attributes->get('_controller'));

        $form = $crawler->selectButton('Preview your job')->form(array(
            'job[company]'      => 'Sensio Labs',
            'job[url]'          => 'http://www.sensio.com/',
            'job[file]'         => __DIR__.'/../../../../../web/bundles/mathildejobeet/images/sensio-labs.gif',
        ...
        $client->submit($form);
        $this->assertEquals('MathildeJobeetBundleControllerJobController::createAction', $client->getRequest()->attributes->get('_controller'));

        $client->followRedirect();
        $this->assertEquals('MathildeJobeetBundleControllerJobController::previewAction', $client->getRequest()->attributes->get('_controller'));

        $kernel = static::createKernel();
        $kernel->boot();
        $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');
 
        $query = $em->createQuery('SELECT count(j.id) from MathildeJobeetBundle:Job j WHERE j.location = :location AND j.is_activated IS NULL AND j.is_public = 0');
    ...
    public function testPublishJob()
    {
        ...
        $query = $em->createQuery('SELECT count(j.id) from MathildeJobeetBundle:Job j WHERE j.position = :position AND j.is_activated = 1');
    ...
    public function testExtendJob()
    {
        ...
        $kernel->boot();
        $em = $kernel->getContainer()->get('doctrine.orm.entity_manager');
        $job = $em->getRepository('MathildeJobeetBundle:Job')->findOneByPosition('FOO5');
